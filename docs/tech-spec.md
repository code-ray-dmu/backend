...
- 질문은 `analysis_run` 단위로 생성하며, 각 실행에서 생성할 질문 수는 환경 변수 `MAX_QUESTIONS_PER_ANALYSIS_RUN`으로 관리한다 (기본값 3). applicant 기준 총 질문 수 상한은 `선택된 저장소 수 × MAX_QUESTIONS_PER_ANALYSIS_RUN`이다.
...
LLM이 생성한 면접 질문. `category`(`SKILL` 또는 `CULTURE_FIT`), `question_text`, `intent`, `priority`를 포함한다. 질문은 `AnalysisRun` 단위로 생성되며 하나의 `AnalysisRun`에는 `MAX_QUESTIONS_PER_ANALYSIS_RUN` 이하의 질문만 저장한다. 지원자 단위로도 조회 가능하도록 `applicant_id`를 직접 저장하며, applicant 기준 총 질문 수 상한은 `선택된 저장소 수 × MAX_QUESTIONS_PER_ANALYSIS_RUN`이다.
...
초기 기본 템플릿 3종은 `libs/integrations/src/llm/docs/prompt-templates-v1.md`를 원천으로 하는 저장소 내 SQL seed 데이터로 관리한다. 해당 seed는 Postgres 컨테이너가 데이터 볼륨이 비어 있는 상태에서 처음 초기화될 때 자동 적재되며, 기존 데이터 볼륨을 재사용하는 경우에는 다시 실행되지 않는다.
...
- Phase 3 기준 필수 범위는 request queue 발행/소비 경로
- 재시도 큐 및 dead-letter 큐 운영은 Phase 4 이후 운영 정책으로 확장 가능
...
**책임**
프롬프트 템플릿 CRUD. 활성 버전 관리, 목적별 조회. 기본 3종 템플릿은 저장소 내 SQL seed로 초기 적재한다.
...
**Worker 연결**: Worker가 각 분석 단계에서 해당 `purpose`의 활성 템플릿을 DB에서 직접 조회하여 사용. 초기 기본값은 Docker seed로 제공하되, 이후 운영 변경은 DB 직접 관리 원칙을 따른다.
...
**책임**
분석 실행 메시지 수신 진입점. Phase 3에서는 실행 시작 책임을 고정하고, 단계 2 이후 파이프라인 오케스트레이션은 Phase 4 책임으로 분리한다.

- Phase 3: 메시지 수신 후 `AnalysisRunJob` 호출, `analysis_runs.status = IN_PROGRESS`, `startedAt = now()` 업데이트, 시작 실패 처리
- Phase 4: `github-repository.processor`, `llm-analysis.processor`를 호출하여 단계 2 이후 전체 파이프라인 오케스트레이션
- 성공: 상세 파이프라인 완료 후 `status = COMPLETED`, `completedAt = now()`
- 실패: 현재 단계 기준 `status = FAILED`, `failure_reason`에 에러 내용 저장

이 processor는 Worker 진입점을 책임하며, Phase 4 완료 시 단계 2 이후 세부 파이프라인을 하위 processor에 위임한다.
...
- **질문 생성 (QUESTION_GENERATION)**: 그룹 컨텍스트 + 코드 분석 결과 + 활성 프롬프트 템플릿(`purpose = 'question_generation'`)으로 LLM에 질문 생성 요청. 각 `analysis_run`당 최대 `MAX_QUESTIONS_PER_ANALYSIS_RUN`개의 질문만 `generated_questions`에 저장한다.
...
- `generated_questions`: 면접 질문 결과 저장. 카테고리/우선순위 포함. 각 `analysis_run`당 최대 `MAX_QUESTIONS_PER_ANALYSIS_RUN`개까지 저장.
...
- `repository_files.raw_analysis_report`는 선택 파일의 원문을 저장한다. 개별 파일 content는 **최대 100KB**로 제한한다. 초과 파일은 저장/전달 대상에서 제외한다.
- `code_analysis.raw_analysis_report`는 LLM의 최종 분석 결과 전문이다.
- `refresh_tokens`: 만료 및 폐기된 토큰 cleanup은 운영 확장 항목으로 분리한다. 현재 `apps/worker/src/schedulers/cleanup.scheduler.ts`의 책임은 stale `analysis_runs` 정리다.
...
Routing Key: `analysis.run.requested`

### 8.3 Worker 처리 흐름

Phase 3의 완료 기준에서 Worker는 "메시지를 받아 분석 실행을 시작하고 상태를 전이하는 것"까지만 책임진다. 아래 단계 중 **단계 0-1은 Phase 3의 확정 범위**이고, **단계 2 이후는 Phase 4 구현 범위**다. 현재 코드에 단계 2 이후 오케스트레이션이 일부 존재하더라도, 이는 선반영 상태이며 Phase 3 완료 판정 기준에는 포함하지 않는다.
...
이 시점까지가 Phase 3의 Worker 시작 경로다. 이 단계에서 progress cache 저장, lock 처리, 실패 시 `FAILED` 전환 규칙을 함께 확정한다.

#### 단계 2 이후: Phase 4 분석 파이프라인

아래 단계는 Worker의 상세 분석 파이프라인이며, `analysis-run.processor` 오케스트레이션과 GitHub/LLM processor 책임 검증은 Phase 4에서 완료한다.
...
- `code_analysis` 레코드 저장: `raw_analysis_report = code_summary 템플릿의 structured output(JSON 문자열) 그대로 저장`
...
- `code_analysis.raw_analysis_report` 조회 (`code_summary` 단계에서 저장한 JSON 문자열을 그대로 전달)
- 활성 프롬프트 템플릿(`purpose = 'question_generation'`) 조회
- LLM에 질문 생성 요청 (실행당 최대 `MAX_QUESTIONS_PER_ANALYSIS_RUN`개 반환)
...
- LLM 응답을 JSON 파싱:
  ...
  ```
- 파싱 결과가 `MAX_QUESTIONS_PER_ANALYSIS_RUN`를 초과하면 `priority` 오름차순 기준 상위 N개만 저장 대상으로 사용한다.
...
- Phase 3 필수 범위는 request queue(`analysis.run.requested`) 발행/소비 경로다.
- `analysis.run.retry`, `analysis.run.deadletter`는 Phase 4 운영 정책 확장을 위한 예약 토폴로지다.
- 실제 재시도 횟수, dead-letter 후처리, 재처리 멱등성 규칙은 후속 Phase에서 확정한다.
...
`code_summary` 단계의 structured output은 검증 후 JSON 문자열 그대로 `code_analysis.raw_analysis_report`에 저장한다. 이후 `question_generation` 단계는 이 값을 다시 입력으로 사용한다.
...
- Routing Key: `analysis.run.requested`
- Queue: `analysis.run.retry` ← Phase 4 재시도 운영 정책에서 사용 예정(예약)
- Queue: `analysis.run.deadletter` ← Phase 4 재시도 초과 격리 정책에서 사용 예정(예약)

> **[server-spec 편차 명시 — 큐 구조]** `server-spec.md` §11.1은 `question.generation.requested` 큐를 별도로 정의하고 있다. 그러나 tech-spec에서는 이 큐를 사용하지 않는다. 그 이유는 `analysis-run.processor`가 파이프라인 전체를 오케스트레이션하며, `llm-analysis.processor`를 내부 메서드 호출로 직접 위임하기 때문이다. 별도 큐를 추가하면 파이프라인 상태 추적과 실패 처리가 복잡해지므로 MVP에서는 단일 request 큐 중심 설계를 선택한다. `analysis.run.retry`, `analysis.run.deadletter`는 Phase 4 운영 정책 전환을 위한 예약 토폴로지로만 유지한다.
>
...
`analysis-run.publisher.ts`는 `libs/integrations/src/rabbitmq/publishers/`의 RabbitMQ 클라이언트를 사용하여 `code-ray.analysis` exchange에 `analysis.run.requested` routing key로 메시지를 발행한다.
...
메시지 ack 정책:
- 처리 성공: ack
- Phase 3 기본 정책: 시작 처리 성공 시 ack, 시작 처리 실패 시 실패 상태를 기록하고 consume 경로에서 오류를 surface
- 재시도 큐 라우팅과 dead-letter 이동은 Phase 4 이후 운영 정책으로 확장
...
초기 기본 템플릿 3종(`file_selection`, `code_summary`, `question_generation`)은 저장소 내 SQL seed로 관리하며, 원문 소스는 `libs/integrations/src/llm/docs/prompt-templates-v1.md`다. 이 seed는 Postgres 컨테이너 최초 초기화 시 자동 적재되고, 기존 데이터 볼륨을 재사용하면 다시 실행되지 않는다.
...
활성 템플릿 조회 결과가 유효하지 않으면 Worker는 아래 식별자로 즉시 실패 처리한다.

- 활성 템플릿 0건: `PROMPT_TEMPLATE_NOT_FOUND: purpose={purpose}`
- 활성 템플릿 2건 이상: `PROMPT_TEMPLATE_ACTIVE_AMBIGUOUS: purpose={purpose} count={n}`
...
- 동일 `purpose`에 `is_active = true`가 복수 존재하지 않도록 DB 제약(예: partial unique index)으로 관리하고, Worker는 방어적으로 애플리케이션 레벨 검증을 함께 수행한다.

### 10.2.1 초기 템플릿 공급

`file_selection`, `code_summary`, `question_generation` 기본 템플릿 3종(v1)은 배포 시점에 seed 또는 migration으로 선적재한다.

- 최소 보장값: 각 `purpose`당 `version = 1`, `is_active = true` 1건
- 운영 원칙: Worker 실행 전에 템플릿 선적재가 완료되어야 하며, 누락 시 Worker는 `PROMPT_TEMPLATE_NOT_FOUND`로 실패한다.
...
- 질문 생성 단위는 `analysis_run` 기준이며 한 실행에서 최대 `MAX_QUESTIONS_PER_ANALYSIS_RUN`개의 질문만 반환해야 한다
- applicant 기준 총 질문 수 상한은 `선택된 저장소 수 × MAX_QUESTIONS_PER_ANALYSIS_RUN`이다
...
- `repository_files` 저장 시 개별 파일 content 크기 제한은 **최대 100KB**로 확정한다. 초과 파일은 저장/전달 대상에서 제외한다.
- 분석 대상 저장소 수는 `MAX_REPO_SELECTION_COUNT`로 제한한다.
- 질문 생성 수는 `MAX_QUESTIONS_PER_ANALYSIS_RUN`으로 제한한다. 이 값을 프롬프트에 명시하여 LLM이 초과 생성하지 않도록 지시한다.
...
3. Worker App 기반 셋업 (RabbitMQ consumer 연결, 실행 시작 경로 확정)
4. Redis 연동 (lock, progress cache 키 관리)
...
6. `analysis-run.processor`: 단계 2 이후 전체 파이프라인 오케스트레이션
7. 실패 처리 및 Dead-letter 큐 연동

### Phase 5: 부가 기능 및 마무리 (1주)

1. `prompt-templates` 운영 및 CRUD 정비
...
### 15.4 결정: 분석 실행당 질문 수 환경 변수화

- 질문 생성 수는 `analysis_run` 단위로 제한한다.
- 실행당 최대 질문 수는 환경 변수 `MAX_QUESTIONS_PER_ANALYSIS_RUN`으로 관리한다.
- 기본값은 3으로 설정한다.
- 질문 생성 단계에서 LLM이 이 상한을 초과해 반환하면 `priority` 오름차순 기준 상위 N개만 저장한다.
...
`prompt_templates`의 CRUD 엔드포인트는 일반 인증 사용자에게 노출하지 않는다. 내부 운영 담당자가 직접 DB를 통해 관리하거나, 별도 내부망 전용 인터페이스로 접근한다. 초기 기본 3종 템플릿은 Docker seed로 자동 주입하되, 이후 운영 변경은 수동 관리 원칙을 따른다.
...
`apps/worker/src/schedulers/cleanup.scheduler.ts`에서 일정 시간 이상 `IN_PROGRESS` 상태를 유지하는 `analysis_runs` 레코드를 `FAILED` 처리하는 스케줄러를 구현한다.

- cleanup 실행 주기: `ANALYSIS_CLEANUP_INTERVAL_SECONDS` (기본값 300초)
- stale 판정 기준: `ANALYSIS_STALE_RUN_THRESHOLD_SECONDS` (기본값 1800초)
- 실행당 정리 배치 크기: `ANALYSIS_CLEANUP_BATCH_SIZE` (기본값 50)
- stale run 실패 사유 식별자: `ANALYSIS_RUN_STALE_TIMEOUT`
- `analysis.run.retry`, `analysis.run.deadletter`의 실제 소비/재처리 정책은 Phase 4 범위에서 제외하고 후속 Phase에서 구현한다.
...
- **분석 동시 실행 제한**: Redis lock TTL **10분**. lock은 파이프라인 전체 동안 유지하고 heartbeat로 TTL을 연장한다.
- **파일 content 크기 상한**: 개별 파일 원문 최대 **100KB** (`MAX_FILE_SIZE_KB=100`). 초과 파일은 저장/전달 대상에서 제외
- **분석 실행당 질문 수 상한**: 기본값 **3** (`MAX_QUESTIONS_PER_ANALYSIS_RUN=3`)
- **`llm_messages` 보존 기간**: MVP 단계에서 **영구 보존**. 운영 확장 시 아카이빙 정책 별도 수립
- **재시도/dead-letter 운영 정책**: `analysis.run.retry`, `analysis.run.deadletter`는 예약 토폴로지로만 유지한다. 실제 소비/재처리 규칙은 후속 Phase에서 확정한다.

### Task 10 완료 기록

- Worker 핵심 경로 성공/실패 테스트를 `analysis-run.job.spec.ts`, `analysis-run.processor.spec.ts`, `worker-orchestration.e2e-spec.ts` 조합으로 닫았다.
- `worker-orchestration.e2e-spec.ts`는 `AppModule` 부팅 smoke test가 아니라 `analysis.run.requested` consumer에서 `AnalysisRunJob`까지 이어지는 queue-to-job 오케스트레이션 회귀 테스트다.
- `server-spec`, `api-spec`의 Phase 4 결정사항을 최신 구현과 맞췄다.
- 남은 운영 정책 미결정 사항은 재시도/dead-letter 소비 규칙으로 한정되며, 후속 Phase 결정 항목으로 유지한다.
